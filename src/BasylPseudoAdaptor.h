...
public:
	...
	virtual void OddSwap(std::vector<unsigned __int64>& gen, int pos1, int pos2);

	//Shuffles things around to enhance the generation scheme. By default it does nothing.
	virtual void Shuffle(std::vector<unsigned __int64>& gen, int round);
	...
	virtual unsigned __int64 SeedFunction(unsigned __int64 pos, unsigned __int64 seed);
};

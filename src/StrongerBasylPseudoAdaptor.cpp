...
int StrongerBasylPseudoAdaptor::Layers(vector<unsigned __int64>& gen, unsigned __int64 current, unsigned __int64 depth)
{
	if (depth <= 0)
	{
	...
	return Layers(gen, gen[(int)(current % (unsigned __int64)gen.size())], depth - 1);
...
void StrongerBasylPseudoAdaptor::Shuffle(vector<unsigned __int64>& gen, int round)
{
	if (round % 100 == 2)
	{
		for (int i = 0; i < gen.size(); i++)
		{
			...
			unsigned __int64  otherPosition = Layers(gen, gen[i], ((temporary << 2) ^ (temporary >> 2)) % 8 + 1);

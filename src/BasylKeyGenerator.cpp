...
BasylKeyGenerator::BasylKeyGenerator(const BasylKeyGenerator &b) :
    _Key1(b._Key1),
    _Key2(b._Key2),
    key1Random(b.key1Random),
    key2Random(b.key2Random),
    hash(b.hash)
{
}
    ...
    _Key2(0)
{
    ...
    Init(pass, INITIAL, ROUNDS, LEFTOFF, EXPANSION, ADDITIONALKEY,
         BasylArray<byte> ((byte *) hash, 32),
         BasylArray<byte> ((byte *) Key1Random, 4),
         BasylArray<byte> ((byte *) Key2Random, 4), true);
}

BasylKeyGenerator::BasylKeyGenerator(string pass, int initial, int rounds,
                                     int leftoff, int expansion,
                                     string additionalKey, ifstream *source) :
    _Key1(0),
    _Key2(0)
{
    char    hash[32];
...
BasylKeyGenerator::BasylKeyGenerator(string pass, int initial, int rounds,
                                     int leftoff, int expansion,
                                     string additionalKey, BasylArray<byte> hash,
                                     BasylArray<byte> key1,
                                     BasylArray<byte> key2, bool encrypted) :
    _Key1(0),
    _Key2(0)
{
    Init(pass, initial, rounds, leftoff, expansion, additionalKey, hash, key1,
         key2, encrypted);
}
...
BasylKeyGenerator::BasylKeyGenerator(string pass, int initial, int rounds,
                                     ...
                                     BasylArray<byte> key1,
                                     BasylArray<byte> key2, bool encrypted,
                                     shared_ptr<BasylPseudoAdaptor> adaptor) :
    _Key1(0),
    _Key2(0)
{
    Init(pass, initial, rounds, leftoff, expansion, additionalKey, hash, key1,
         key2, encrypted, adaptor);
}
...
void BasylKeyGenerator::Init(string pass, int initial, int rounds, int leftoff,
                             int expansion, string additionalKey)
{
    byte    n[4],
            q[4];
    GetRandom(n, 4);
    GetRandom(q, 4);
    Init(pass, initial, rounds, leftoff, expansion, additionalKey,
         GetRandomizedHash(), BasylArray<byte> (n, 4), BasylArray<byte> (q, 4),
         false);
}

void BasylKeyGenerator::Init(string pass, int initial, int rounds, int leftoff,
                             int expansion, string additionalKey,
                             BasylArray<byte> hash, BasylArray<byte> key1,
                             BasylArray<byte> key2, bool encrypted)
{
    Init(pass, initial, rounds, leftoff, expansion, additionalKey, hash, key1,
         key2, encrypted, make_shared<BasylPseudoAdaptor> ());
}

...
public:
    ...
    static void Encrypt(string fileName, string outputName, string pass,
                        int initial, int rounds, int leftoff, int expansion,
                        string additional)
    {
        ifstream    in(fileName, ios::binary);
        ofstream    out(outputName, ios::binary);
        Encrypt(in, out, BasylKeyGenerator(pass, initial, rounds, leftoff,
                expansion, additional));
    }

    static void Encrypt(string fileName, string pass, int initial, int rounds,
                        int leftoff, int expansion, string additional)
    {
        Encrypt(fileName, fileName + ".bes", pass, initial, rounds, leftoff,
                expansion, additional);
    }
                        ...
                        string additional)
    {
        ...
        Decrypt(in, out, BasylKeyGenerator(pass, initial, rounds, leftoff,
                expansion, additional, &in));
    }
    ...
    static void Decrypt(string fileName, string pass, int initial, int rounds,
                        int leftoff, int expansion, string additional)
    {
        Decrypt(fileName, fileName.substr(0, fileName.find_last_of('.')), pass,
                initial, leftoff, rounds, expansion, additional);
    }
